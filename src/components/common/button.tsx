    ...
    return (
        <>
          <a href="#" className="relative inline-block text-lg group">
                <span className="relative z-10 block px-5 py-3 overflow-hidden font-medium leading-tight text-gray-800 transition-colors duration-300 ease-out border-2 border-gray-900 rounded-lg text-center">
                  <span className={`absolute inset-0 w-full h-full px-5 py-3 rounded-lg bg-gray-50 ${bgColor}`} />
                  ...
                  <span className={`relative font-noto font-bold ${btnTextColor}`}>{buttonText}</span>
                </span>
                <span
                  className={`absolute bottom-0 right-0 w-full h-12 -mb-1 -mr-1 transition-all duration-200 ease-linear bg-gray-900 rounded-lg   ${btnBgShadow}`}
